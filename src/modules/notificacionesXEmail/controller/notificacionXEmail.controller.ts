...
import { notificacionService } from '../service/notificacionXEmail.service.js';
...
export const notificacionXEmailController = {
  ...
  create: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const parsed = CreateNotificacionXEmailDto.safeParse(req.body);
      if (!parsed.success) {
  ...
  update: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const parsed = UpdateNotificacionXEmailDto.safeParse(req.body);
      if (!parsed.success) {
        return respondZodError(res, parsed.error);
      }
